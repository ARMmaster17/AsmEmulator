...
// ADD Add
// Page 261 of the Intel 80386 Programmers Reference Manual.
void Opcodes::ADD(SFLAG_Register &sfr, GEN_Register &gr, RAM &mem, std::vector<std::string> &cmd) {
    // TODO: Find a way to expand out to correct opcode depending on argument types.
    // For now, just do registers.
    gr.setReg_8(cmd[1], gr.getReg_8(cmd[1]) + gr.getReg_8(cmd[2]));
}
